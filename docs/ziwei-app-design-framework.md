...
八字与紫微共用 L2 归一化出生时刻(夏令时扣除、真太阳时校正后的本地时刻),晚子时日柱归属沿用流派 `dayDivide`,保证两套体系永不因时刻处理不一致而"各说各话"。

### 5.2.1 历法层:直接集成成熟公开框架

- 内核为 `lunar-typescript`(6tail,MIT),为 JS 生态通行的历法/八字库:年柱以立春、月柱以十二节为界,时柱五鼠遁,`sect` 控制晚子时日柱归属;同时提供纳音、十二长生、旬空、胎元/命宫/身宫、大运起运(阳男阴女顺行,3 日折 1 年)与流年。
- 适配层 `packages/core/src/bazi/calendar.ts` 把中文干支反查为注册表 key,输出可序列化结构;库表(藏干/纳音)在测试中与本库表交叉核对。
- **语义边界**:`Astrolabe.ganzhi` 是紫微排盘干支(年按流派分界、月按农历月),是安星与生年四化的依据,但不是节气四柱;标准四柱一律取 `engine.bazi(chart).pillars`。中宫显示标准四柱,与排盘年干不同时另行注明。

### 5.2.2 分析层:古法规则显式建模

| 模块 | 方法 | 依据 |
|---|---|---|
| 藏干十神 | 本气/中气/余气表;十神由日干与他干五行生克 × 阴阳同异推得 | 《渊海子平》《三命通会》 |
| 日主旺衰 | 得令(月令旺相休囚死,40)+ 得地(四支藏干通根,按气与柱位加权,30)+ 得势(年月时干生扶/克泄耗,30);阈值 72/56/44/28 | 通行五行权重简化法,结果附算法说明 |
| 格局 | 月支本气→中气→余气首个透干者取格,皆不透取本气;比肩→建禄,劫财→羊刃(阳)/月劫(阴);从强/从弱仅作候选 | 《子平真诠》 |
| 用神 | 扶抑法为主(强者克泄耗、弱者生扶,中和补最弱),调候为辅(亥子丑月用火、巳午未月用水) | 《滴天髓》《穷通宝鉴》 |
| 神煞 | 天乙/太极/文昌贵人、禄神、羊刃、桃花、驿马、华盖、将星、劫煞、亡神、孤辰寡宿、魁罡、天德月德、红艳、空亡(18 项),每项附查法依据 | 《三命通会·神煞》通行歌诀 |
| 干支关系 | 天干五合/四冲;地支六合、三合(全/半)、三会、六冲、相刑、自刑、相害、相破,标注柱位与冲提纲 | 通行表 |

所有分析结果都是"算法初判",Prompt 中明示可在论证后修正但须说明理由;五行缺项只作表面计数,禁止"缺什么补什么"话术。

### 5.2.3 与 L3/L4/L5 的接线

- L3:`bazi` 域知识条目(日主十干、十神、旺衰五级、十一格局、调候、神煞),实体约定 `['bazi', <维度>, <key>]`,由 `baziSignals()` 产生检索信号;`retrieveSignals()` 允许紫微信号与八字信号合并召回。
- L4:`buildBaZiPrompt()` 用于纯八字解读;`buildSystemPrompt(..., { bazi, year })` 用于紫微 + 八字双系统互参,并注入"相合处加重、相悖处并陈"的互参纪律;网关 `/api/interpret` 以 `system` 与 `year` 参数切换,并纳入缓存键。
- L5:星盘页「紫微盘 | 八字盘」切换;八字盘含四柱表、旺衰仪表、五行条、格局用神、神煞关系与大运/流年时间轴(与紫微流年共用年份状态);智能体「命理体系」选择器;导出含八字全量参数。

### 5.2.4 验证

黄金向量取公开命例(2000-01-01 00:30 己卯丙子戊午壬子;毛泽东 1893-12-26 辰时 癸巳甲子丁酉甲辰)与边界(立春/惊蛰切换、晚子时两派、1990-01-15 男逆女顺起运),另以 400 组随机时刻交叉核对紫微排盘干支与标准四柱:日柱 100% 一致,月柱按预期在节气与农历月错位处不同(约 30%),年柱在立春至正月初一窗口不同(约 1.5%)。
