...
- 内置 **39 个**格局定义，支持成立条件、加分条件与破格条件。
- 合盘包含命宫关系、年支合冲刑害、生年四化互飞及夫妻宫互参等确定性结果。
- 八字（四柱）与紫微共用同一校正出生时刻：基于 `lunar-typescript` 排四柱（立春分年、节气分月、晚子时归属可配置），并实现藏干十神、日主旺衰（月令/通根/生扶分项）、月令取格、扶抑与调候用神、十八神煞、干支合冲刑害破与大运流年。
- 支持完整命盘、分析结果与八字参数导出为 JSON，供外部智能体或研究工具继续处理。

### 3. 可溯源知识库

- 使用 Zod 对知识条目进行结构化校验。
- 当前包含 **433 条**知识条目，覆盖星曜、宫位、四化、组合、格局、运限与八字（日主、十神、旺衰、格局、调候、神煞）等领域。
- 每条知识包含来源等级、出处、流派、置信度与审核状态。
- 内置 **20 类**解读技法，包括整体、婚恋、事业、生意、学业、健康、财帛、子女、父母、兄弟、人际、迁移、福德、大限、流年、八字命理、八字大运流年，以及伴侣合盘、家庭群盘、合伙团队三个群盘技法。
...
- 支持单盘技法解读、自由提问、双盘合盘、任意 2~6 份档案组合的群盘（两两关系矩阵）解读，紫微 / 八字 / 双系统互参三种命理体系与多轮对话历史；推理型模型的思考过程与正文分流（`reasoning_content` 字段与 `<think>` 标签均支持）。
...
- APK 使用稳定签名密钥与递增版本号构建，可直接覆盖安装升级、数据保留；「设置 → 数据备份」可导出/导入全部本机数据。
...
| 排盘内核 | iztro 2.5.8（紫微）、lunar-typescript 1.8.6（八字历法） |
| 数据校验 | Zod 3 |
...
|---|---:|
| TypeScript 类型检查 | 通过 |
| 自动化测试 | **29 个测试文件，247 项测试全部通过** |
| Web 生产构建 | 通过 |
| 黄金命例回归 | 55 项 |
| 城市经纬度记录 | 3339 条 |
| 格局定义 | 39 个 |
| 知识条目 | 433 条 |
| 知识审核状态 | 426 draft / 5 reviewed / 2 verified |
| 解读技法 | 20 类 |
